...
def get_IPC(descriptor_data, sim_path, output_dir):
  ...
  try:
    for config_key in descriptor_data["configurations"].keys():
      ...
      for benchmark in benchmarks_org:
        ...
        with open(exp_path+config_key+'/bp.stat.0.csv') as f:
          lines = f.readlines()
          for line in lines:
            if 'BTB_ON_PATH_MISS_count' in line:
              tokens = [x.strip() for x in line.split(',')]
              BP_MISS = float(tokens[1])
            if 'BTB_ON_PATH_HIT_count' in line:
              tokens = [x.strip() for x in line.split(',')]
              BP_HIT = float(tokens[1])  
              break

        avg_IPC_config += (BP_MISS / (BP_MISS + BP_HIT))

        cnt_benchmarks = cnt_benchmarks + 1
        if len(benchmarks_org) > len(benchmarks):
          benchmarks.append(benchmark_name)

        ipc_config.append((BP_MISS / (BP_MISS + BP_HIT)))
    ...
    plot_data(benchmarks, ipc, 'BP Miss Ratio', output_dir+'/Always_BP.png')
